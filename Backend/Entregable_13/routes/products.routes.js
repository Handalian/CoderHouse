...
routerProducts.get('/', async (req, res) => {
    if (req.query.logoutbutton) {
        res.redirect('/api/user/logOut')
    }
    const allProducts = await productsDB.loadProducts();
    res.render('pages/mainView', { user: req.session.user, email: req.session.email, allProducts: allProducts });
});
