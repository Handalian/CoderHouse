import fs from 'fs';
import { classProductos as ContenedorProductos } from "../../daos/productos/ClassProductosArchivo.js";
export class classCarts {
    constructor(route) {
        this.route = route;
    }
    async loadFiles() {
    ...
    async addProduct(idProducto, idCarrito) {
        const carts = await this.loadFiles();
        const products = new ContenedorProductos('./src/db/productos.json');
